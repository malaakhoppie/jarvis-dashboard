"""
Settings — API keys, service connections, Airtable config, and app preferences.
"""
import os
import requests
...
_TABLE_IDS = {
    "trades":        "tblDREH4UqYWPKLRs",
    "daily_summary": "tblg19WnSmvHyzCPf",
    "pattern_flags": "tblH78uuqnCTRkGPt",
}
...
def render_settings():
    ...
    for col, (name, tid) in zip([t1, t2, t3], _TABLE_IDS.items()):
        col.markdown(
    ...
    if tc1.button("Test Read", use_container_width=True):
        if not airtable_key or airtable_key.startswith("your_"):
            ...
            for name, tid in _TABLE_IDS.items():
                try:
                    r = requests.get(
    ...
    if tc2.button("Test Write (dry-run)", use_container_width=True):
        if not airtable_key or airtable_key.startswith("your_"):
            ...
            # Test write with a clearly invalid payload — if we get 422 back it means write IS permitted
            # If we get 403 back, the token is read-only
            try:
                r = requests.post(
                    f"https://api.airtable.com/v0/{airtable_base}/{_TABLE_IDS['trades']}",
                ...
                if r.status_code == 403:
                    st.error(
                        "WRITE BLOCKED — token is missing data.records:write scope. "
                        "Go to airtable.com/create/tokens → edit token → add data.records:write → save."
                    )
    ...
    st.divider()
    st.markdown(
        "<div style='color:#44445a;font-size:0.82rem'>Jarvis v1.0 · June 2026</div>",
        unsafe_allow_html=True,
    )
